...
@pytest.mark.unit
async def test_grounded_answer_extracts_complete_ordered_context() -> None:
    answer = await providers.OllamaChatProvider("http://local", "chat", 10).answer(
        "Trace the complete process", "[1] Report\n1. First\n2. Second\n3. Third", 1
    )

    assert [item.text for item in answer.claims] == ["First", "Second", "Third"]
    assert answer.style == "steps"


def test_complete_answer_items_cover_tables_and_inline_lists() -> None:
    assert providers._required_context_items(
        "List every field", "|Field|Purpose|\n|---|---|\n|hash|identity|\n|page|location|"
    ) == ["hash", "page"]
    assert providers._required_context_items(
        "Specify every field", "hash + page + coordinates + fingerprint"
    ) == ["hash", "page", "coordinates", "fingerprint"]


def test_grounding_rejects_claims_with_unsupported_terms() -> None:
    answer = GroundedResponse(
        claims=[
            GroundedClaim(
                text="Chunk IDs are generated by the model during an evaluation run",
                citations=[1],
            )
        ]
    )
...
def test_extractive_fallback_selects_query_relevant_source_paragraph() -> None:
    context = (
        "[1] report.pdf, page 12\nDo not store production chunk IDs as gold evidence. "
        "Chunk IDs belong in evaluation run outputs.\n\nUnrelated monitoring guidance."
    )

    assert providers._extractive_fallback("Why are chunk IDs not gold labels?", context) == (
        "Do not store production chunk IDs as gold evidence. "
        "Chunk IDs belong in evaluation run outputs."
    )
